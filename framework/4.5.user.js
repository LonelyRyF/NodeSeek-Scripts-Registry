...
(function() {
    ...
    // 自动更新检查间隔（毫秒），默认 24 小时
    const UPDATE_CHECK_INTERVAL = 24 * 60 * 60 * 1000;

    // ==========================================
    // fetch 封装（带分钟级缓存 buster）
    // ==========================================
    ...
    const SelfUpdater = {
        ...
        shouldCheck() {
            const last = GM_getValue('ns_self_update_checked_at', 0);
            return (Date.now() - last) > UPDATE_CHECK_INTERVAL;
        },

        async check() {
            if (!this.shouldCheck()) return null;
            try {
                const text = await gmFetch(VERSION_URL);
                ...
                if (compareVersions(info.version, CURRENT_VERSION) > 0) {
                    console.log(`[NS UI] 发现新版本 v${info.version}，当前 v${CURRENT_VERSION}`);
                }
                return info;
            } catch(e) {
                console.warn('[NS UI] 自动更新检查失败:', e.message);
                return null;
            }
        },
        ...
        getCached() {
            ...
            try {
                const raw = GM_getValue('ns_self_update_info', null);
        ...
        renderBanner(container) {
            if (!container) return;
            const info = this.getCached();
            if (!info || !this.hasUpdate()) return;
            const existing = container.querySelector('#ns-self-update-banner');
            if (existing) return;
    ...
    const Market = {
        ...
        async install(item, onProgress) {
            ...
            // 转义反引号和模板插值符，防止 _execute 中字符串拼接时语法错误
            // 注意：反斜杠必须最先处理，否则后续生成的转义符会被二次转义
            ...
            const data = {
                ...
                version: item.version || '0.0.0',  // 直接使用 registry.json 中的版本号
                author: item.author || '',
                tags: item.tags || [],
                url: item.url,
                scriptContent: safeContent,
                installedAt: Date.now()
                // enabled 字段已移除，启用状态统一由模块管理中心的 moduleStates 控制
            };
        ...
        _execute(data) {
            // 检查模块管理中心的启用状态，禁用则拦截执行
            if (moduleStates[data.id] === false) {
                console.log('[NS Market] 模块已在管理中心禁用，跳过执行:', data.id);
                return;
            }
            ...
            // 等待 DOM 就绪后再执行，避免 @run-at document-start 导致的竞态问题
            script.textContent = `
(function() {
    var _exec = function() {
    ...
    if (document.readyState === 'loading') {
        ...
        autoStart() {
            ...
            Object.values(installed).forEach(data => {
                // 检查模块管理中心的启用状态，禁用则跳过
                if (moduleStates[data.id] === false) {
                    console.log('[NS Market] 模块已在管理中心禁用，跳过自动启动:', data.id);
                    return;
                }
        ...
        async silentUpdate() {
            ...
            try {
                ...
                console.warn('[NS Market] 静默更新检查失败:', e.message);
                return;
            }
            ...
            for (const item of scripts) {
                ...
                if (compareVersions(item.version, inst.version) > 0) {
                    console.log(`[NS Market] 静默更新: ${item.name} ${inst.version} → ${item.version}`);
                    try {
                        await this.install(item, () => {});
                    } catch(e) {
                        console.warn(`[NS Market] 静默更新失败 (${item.id}):`, e.message);
                    }
                }
            }
        },
        ...
        async renderUI(container) {
            ...
            const loadingEl = UI._el('div', {
                ...
                html: `<span style="color:#888;font-size:14px;">正在加载脚本列表...</span>`
            });
    ...
    const coreAPI = {
        ...
        init() {
                    ...
                    c.innerHTML = `
                        <fieldset>
                            ...
                            <div style="text-align: center; padding: 40px 0;">
                                <span style="color: #888; font-size: 14px;">正在加载模块...</span>
                            </div>
                        </fieldset>
                    `;
                    setTimeout(() => {
            ...
            // 后台静默检查：超过 24 小时则检查基座更新和子脚本更新
            SelfUpdater.check().then(info => {
                if (info) Market.silentUpdate();
            });
