(function(){const API=window.NodeSeekUI;if(!API){console.error('[NS Beautify] 未检测到 NodeSeek UI 基座');return;}
const MODULE_ID='ns_beautify';const STYLE_TAG_ID='ns-beautify-styles';const DEFAULTS={enableBg:true,bgUrl:'https://file.8888998.xyz/2025/09/23/1758618109392-4xswnf.webp',bgBlur:10,enableGlassmorphism:true,glassOpacity:0.45,glassBlur:20,disableBgProtection:false,avatarStyle:'liquid',enableSmoothing:true,enableCustomCursor:false,defaultCursorUrl:'https://npm.elemecdn.com/kang-static/Hexo/img/default.cur',pointerCursorUrl:'https://npm.elemecdn.com/kang-static/Hexo/img/pointer.cur',textCursorUrl:'',enableCustomScrollbar:true,scrollbarColor:'#888888',scrollbarHoverColor:'#555555'};const SCHEMA=[{key:'enableBg',type:'switch',label:'开启全局背景图',default:DEFAULTS.enableBg},{key:'bgUrl',type:'text',label:'背景图 URL',description:'留空则无背景',default:DEFAULTS.bgUrl},{key:'bgBlur',type:'number',label:'背景图模糊度 (px)',min:0,max:50,step:1,default:DEFAULTS.bgBlur},{key:'enableGlassmorphism',type:'switch',label:'启用全局毛玻璃 UI',default:DEFAULTS.enableGlassmorphism},{key:'glassOpacity',type:'number',label:'全局毛玻璃底色透明度',description:'0为纯透明。侧边栏与编辑器受保护区不受此限制。',min:0.0,max:1.0,step:0.1,default:DEFAULTS.glassOpacity},{key:'glassBlur',type:'number',label:'全局毛玻璃模糊度 (px)',description:'数值越大，玻璃背后的图案越模糊',min:0,max:50,step:1,default:DEFAULTS.glassBlur},{key:'disableBgProtection',type:'switch',label:'我认为我不需要底色保护',description:'开启后将彻底移除背景图上的黑/白纱遮罩，实现 100% 裸奔。若文字融入背景导致瞎眼，请自行承担后果！',default:DEFAULTS.disableBgProtection},{key:'avatarStyle',type:'select',label:'头像与元素样式',options:[{text:'默认 (方角)',value:'default'},{text:'纯圆形',value:'circle'},{text:'液态玻璃',value:'liquid'}],default:DEFAULTS.avatarStyle},{key:'enableCustomCursor',type:'switch',label:'启用自定义鼠标指针',default:DEFAULTS.enableCustomCursor},{key:'defaultCursorUrl',type:'text',label:'普通指针 URL',description:'留空则使用系统默认',default:DEFAULTS.defaultCursorUrl},{key:'pointerCursorUrl',type:'text',label:'悬停指针 URL (链接/按钮)',description:'留空则使用系统默认小手',default:DEFAULTS.pointerCursorUrl},{key:'textCursorUrl',type:'text',label:'文本输入指针 URL',description:'打字框的光标，留空则使用系统默认',default:DEFAULTS.textCursorUrl},{key:'enableCustomScrollbar',type:'switch',label:'启用自定义滚动条',default:DEFAULTS.enableCustomScrollbar},{key:'scrollbarColor',type:'color',label:'滚动条颜色',default:DEFAULTS.scrollbarColor},{key:'scrollbarHoverColor',type:'color',label:'滚动条悬停颜色',default:DEFAULTS.scrollbarHoverColor},{key:'enableSmoothing',type:'switch',label:'启用现代化平滑动画',default:DEFAULTS.enableSmoothing}];function applyStyles(){const cfg=API.getConfig(MODULE_ID,SCHEMA);let css='';const baseOpacity=parseFloat(cfg.glassOpacity);const lightOpacity=Math.min(baseOpacity+0.1,0.85);const blurValue=cfg.glassBlur!==undefined?cfg.glassBlur:20;const protectedLightBg=Math.max(0.65,lightOpacity);const protectedDarkBg=Math.max(0.65,baseOpacity);const overlayLightAlpha=cfg.disableBgProtection?0:(cfg.enableGlassmorphism?((1.0-baseOpacity)*0.5):0.3);const overlayDarkAlpha=cfg.disableBgProtection?0:(cfg.enableGlassmorphism?((1.0-baseOpacity)*0.5):0.3);css+=`
            :root {
        ...
        `;if(cfg.enableBg){const currentBgUrl=cfg.bgUrl&&cfg.bgUrl.trim()!==''?cfg.bgUrl:DEFAULTS.bgUrl;css+=`
                body, .bg1, .bg2 {
            ...
            `;}
if(cfg.enableGlassmorphism){const glassmorphismCss=`
            ...
            `;}
if(cfg.avatarStyle==='circle'){css+=` .avatar-wrapper img, .avatar-normal, .avatar-sm { border-radius: 50% !important; } `;}else if(cfg.avatarStyle==='liquid'){css+=`
            ...
            `;}
if(cfg.enableCustomScrollbar){css+=`
            ...
            `;}
if(cfg.enableCustomCursor){const defCur=cfg.defaultCursorUrl?`url('${cfg.defaultCursorUrl}'), auto`:'auto';const ptrCur=cfg.pointerCursorUrl?`url('${cfg.pointerCursorUrl}'), pointer`:'pointer';const txtCur=cfg.textCursorUrl?`url('${cfg.textCursorUrl}'), text`:'text';css+=` 
...
API.register({id:MODULE_ID,name:'全站动态美化引擎',version:'2.3.0',description:'新增底层遮罩强制关闭开关与亮色保护机制。',render:function(container){const currentConfig=API.getConfig(MODULE_ID,SCHEMA);const form=API.UI.buildConfigForm(SCHEMA,currentConfig,function(newConfig){API.store(MODULE_ID,'config',newConfig);applyStyles();});container.appendChild(form);},execute:function(){applyStyles();},onToggle:function(enabled){enabled?applyStyles():API.removeStyle(STYLE_TAG_ID);}});})();