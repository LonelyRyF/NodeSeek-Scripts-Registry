(function() {
    ...
    function injectStyles(cfg) {
        ...
        if (cfg.avatarMode === 'blur') {
            css += '[data-ns-privacy="avatar"] { filter: blur(' + cfg.blurAvatarAmount + 'px) !important; transition: filter 0.2s !important; }';
        }
        if (cfg.nameMode === 'blur') {
            css += '[data-ns-privacy="name"] { filter: blur(' + cfg.blurNameAmount + 'px) !important; transition: filter 0.2s !important; user-select: none !important; }';
        }
        if (css) {
            API.addStyle(css, STYLE_ID);
        }
    }

    function applyToAvatar(imgEl, cfg) {
        if (!imgEl || cfg.avatarMode === 'none') return;
        imgEl.setAttribute('data-ns-privacy', 'avatar');
        if (cfg.avatarMode === 'replace') {
    ...
    function applyToName(el, cfg) {
        if (!el || cfg.nameMode === 'none') return;
        el.setAttribute('data-ns-privacy', 'name');
        if (cfg.nameMode === 'replace') {
            if (!el.getAttribute('data-ns-orig-name')) {
    ...
    function processListItem(item, cfg) {
        ...
        if (cfg.coverAuthor) {
            ...
            var authorLink = item.querySelector('.info-author a');
            if (authorLink) applyToName(authorLink, cfg);
        }
        if (cfg.coverLastCommenter) {
            var lastCommenterLink = item.querySelector('.info-last-commenter a');
            if (lastCommenterLink) applyToName(lastCommenterLink, cfg);
        }
    ...
    function resetAll() {
        ...
        document.querySelectorAll('.post-list-item[data-ns-processed]').forEach(function(item) {
            ...
            var img = item.querySelector('img.avatar-normal');
            if (img) {
                img.style.visibility = '';
            }
            item.querySelectorAll('[data-ns-orig-name]').forEach(function(a) {
        ...
        document.querySelectorAll('.content-item[data-ns-processed]').forEach(function(item) {
            ...
            var nameEl = item.querySelector('.author-name');
            if (nameEl) {
