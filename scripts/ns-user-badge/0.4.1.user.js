    ...
    function injectBadges(authorLink, config) {
        ...
        apiCache.get(uid).then(data => {
            if (data && data.success && data.detail) {
                ...
                const customStyle = `background: transparent; border: 1px solid ${config.badgeColor}; color: ${config.badgeColor}; margin-left: 4px; line-height: 1; padding: 0 4px; font-size: 11px; border-radius: 3px; display: inline-block;`;
