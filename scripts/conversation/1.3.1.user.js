(function () {
    ...
    const injectStyle = () => {
        if (document.getElementById('ns-conv-style')) return;
        const style = document.createElement('style');
        style.id = 'ns-conv-style';
        style.textContent = `.comment-menu[data-v-254da704] { flex-wrap: wrap; }`;
    ...
    const getFloorEl = async (floorId, enableFetch) => {
        ...
        if (docCache.has(page)) return { el: docCache.get(page).getElementById(floorId), page };
        try {
        ...
        const scanRepliesOf = async (targetFloorId, depth) => {
            ...
            if (enableScan && maxScanPages > 0) {
                ...
                for (let p = currentPage + 1; p <= endPage; p++) {
                    if (loadedNums.has(p)) continue;
                    let doc = docCache.get(p);
                    if (!doc) {
                        try {
                            doc = await fetchDoc(pageUrl(p));
                            docCache.set(p, doc);
                        } catch { continue; }
                    }
    ...
    const renderThread = async (triggerItem, cfg) => {
        ...
        try {
            [upChain, downChain] = await Promise.all([
                ...
                buildDownChain(triggerItem, cfg.enableScan, cfg.maxScanPages, cfg.enableNestedScan, cfg.maxNestedDepth)
            ]);
        } catch (e) {
            loading.remove();
    ...
    API.register({
        ...
        onToggle(on) {
            if (on) {
                ...
                cleanup();
                document.getElementById('ns-conv-style')?.remove();
